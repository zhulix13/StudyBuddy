...
import { X, Camera, Info, Lock, Globe, Upload, Loader2 } from "lucide-react";
...
export const DesktopCreateGroupPanel = ({
  isOpen,
  onClose,
  onSuccess,
  desktopBackdropVariants, 
  ...
  // Supabase mutation with proper success/error handling
  ...
  // Handle success modal close - this was missing the proper chain
  const handleSuccessClose = () => {
    setShowSuccessModal(false);
    
    // Call parent success callback and close main modal
  ...
  const handleErrorClose = () => {
    ...
    // Reset the mutation error state
  ...
  const handleRetry = () => {
    setShowErrorModal(false);
    createGroupMutation.reset();
    // Don't automatically retry, let user click submit again
  };

  // Close on escape key
  useEffect(() => {
    const handleEscape = (e: KeyboardEvent) => {
      if (e.key === "Escape" && isOpen && !createGroupMutation.isPending && !showSuccessModal && !showErrorModal) {
        handleClose();
      }
    };
  ...
  }, [isOpen, createGroupMutation.isPending, showSuccessModal, showErrorModal]);
  ...
  return (
    <>
        ...
        {isOpen && (
          <>
            ...
            <motion.div
              ...
              animate="visible"
              exit="exit"
              variants={desktopPanelVariants as any}
      ...
      {/* Success Modal - Fixed to use handleSuccessClose */}
      <SuccessModal
      ...
      {/* Error Modal - Fixed to use proper error handling */}
      {createPortal(
        <ErrorModal
