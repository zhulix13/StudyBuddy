// CreateGroupModalMobile.tsx - Fixed Version
import React, { useState, useRef, useEffect } from "react";
import { motion, AnimatePresence } from "framer-motion";
import { X, Camera, Upload, Loader2, Info, Lock, Globe, ChevronDown, CheckCircle, AlertCircle } from "lucide-react";
...
export const MobileCreateGroupPanel = ({
  isOpen,
  onClose,
  onSuccess,
}: {
  ...
  return createPortal(
    <>
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-[#111827] sticky top-0 z-10 shadow-sm">
            <div className="flex items-center gap-3">
              <button
                ...
                type="button"
                className="p-2 text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200 transition-colors rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 disabled:opacity-50 active:scale-95"
              >
                <X className="w-5 h-5" />
              </button>
          ...
          <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto">
                ...
                <div className="relative">
                  <div className="w-24 h-24 ring-2 ring-gray-200 dark:ring-gray-700 rounded-full overflow-hidden">
                    {avatarPreview ? (
                      ...
                      <div className="w-full h-full bg-gradient-to-br from-blue-500 via-slate-700 to-indigo-600 text-white font-semibold text-3xl flex items-center justify-center">
            ...
            <div className="sticky bottom-0 flex items-center gap-3 p-6 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
              <button
                type="button"
                onClick={handleClose}
                disabled={createGroupMutation.isPending}
                className="flex-1 h-12 text-base bg-transparent border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors disabled:opacity-50 active:scale-95"
              >
                Cancel
              </button>
              <button
                ...
                className="flex-1 h-12 text-base bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium transition-colors disabled:opacity-50 flex items-center justify-center active:scale-95"
              >
      ...
      <ErrorModal
        ...
        error={createGroupMutation.error?.message || "An unexpected error occurred"}
        onRetry={handleRetry}
      />
    </>,
    document.body
